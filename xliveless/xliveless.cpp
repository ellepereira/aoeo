...
{
    TRACE("XSocketIOCTLSocket  (socket = %X, cmd = %d, argp = %d)");
...
// #12: XSocketConnect
int WINAPI XSocketConnect(SOCKET s, const sockaddr_in *name, int namelen)
{
    TRACE("XSocketConnect  (socket = %X, name = %X, namelen = %d) ip: %hs port: %d",
			s, name, namelen, inet_ntoa(name->sin_addr), ntohs(name->sin_port));
...
{
    TRACE("XSocketWSARecv (socket = %X, lpBuffers = %X, dwBufferCount = %d, lpNumberOfBytesRecvd = %X, lpFlags = %X, lpOverlapped = %X, lpCompletionRoutine = %X)", s, lpBuffers, dwBufferCount, lpNumberOfBytesRecvd,  lpFlags, lpOverlapped, lpCompletionRoutine);
...
{
    TRACE("XSocketWSASend (SOCKET s = %X, LPWSABUF lpBuffers = %X, DWORD dwBufferCount = %d, LPDWORD lpNumberOfBytesSent = %d, DWORD dwFlags = %d, LPWSAOVERLAPPED lpOverlapped = %X, LPWSAOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine = %X)",
		s, lpBuffers, dwBufferCount, * lpNumberOfBytesSent, dwFlags, lpOverlapped, lpCompletionRoutine);
	TRACE("-- sending %d bytes", lpBuffers->len);
...
INT WINAPI XSocketWSAGetLastError ()
{
	...
	if( print < 15 )
	{
    TRACE("XSocketWSAGetLastError => %i", lastError);
		print++;
	}
...
{
    TRACE("XNetServerToInAddr (const IN_ADDR ina = %u, DWORD dwServiceId = %d, IN_ADDR * pina = %x)", ina, dwServiceId, pina);
	if(pina)
		memcpy(pina, &ina, sizeof(IN_ADDR));
...
{
    TRACE("XNetConnect (DWORD = %u | %hs)", ina, inet_ntoa(ina));
    return 0;
}
...
int WINAPI XEnumerate(HANDLE hEnum, CHAR *pvBuffer, DWORD cbBuffer, PDWORD pcItemsReturned, PXOVERLAPPED pOverlapped)
{
	...
	if (hEnum == g_dwXtitleContent) {
		...
		memcpy(pvBuffer, &server, sizeof(XTITLESERVER_INFO));
	...
	if( hEnum == g_dwFakeContent && dlcinit != 0x7FFFFFFF )
	{
		...
		TRACE("-xDLC enumerate (marketplaceDlcCount = %d, dlcinit = %d)", marketplaceDlcCount, dlcinit);
...
{
    TRACE("XTitleServerCreateEnumerator (pszServerInfo=> %hs, cItem=> %d)", pszServerInfo, cItem);

	if (pcbBuffer) *pcbBuffer = sizeof(XTITLESERVER_INFO) * cItem;
	if (phEnum)
	{
		*phEnum = g_dwXtitleContent = CreateMutex(NULL, NULL, NULL);


		TRACE("- xTitle Handle = %X", *phEnum);
	}
	...
	DWORD dwDesiredAccess, DWORD dwShareMode, SECURITY_ATTRIBUTES * pSecurityAttributes, DWORD dwCreationDisposition, DWORD dwFlagsAndAttributes, PHANDLE phModule)
{
	...
	else if (lstrcmp(pszFilePath, L"C:\\Users\\Luciano\\AppData\\Local\\Microsoft\\Age of Empires Online\\startup\\game.cfg") == 0){
		wcscpy(dlcfile, L"C:\\Users\\Luciano\\AppData\\Local\\Microsoft\\Age of Empires Online\\startup\\user.cfg");
	}
...
{
    TRACE("XLiveLess_5347 XLoadLibrary %hs", lpLibFileName);
    int result; // eax@2
    HMODULE v6; // eax@3
...
// 5363
DWORD WINAPI XLiveLess_5363(DWORD* a, DWORD* b, DWORD* c, DWORD* d)
{
  TRACE("XLiveLess_5363  (a=%d, b=%d, c=%d, d=%d) c = %hs", *a, *b, *c, *d, c);
